...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'trg_news_translations_updated') THEN
    CREATE TRIGGER trg_news_translations_updated
    BEFORE UPDATE ON public.news_translations
    FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'trg_ebooks_translations_updated') THEN
    CREATE TRIGGER trg_ebooks_translations_updated
    BEFORE UPDATE ON public.ebooks_translations
    FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'trg_events_translations_updated') THEN
    CREATE TRIGGER trg_events_translations_updated
    BEFORE UPDATE ON public.events_translations
    FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'trg_technical_materials_translations_updated') THEN
    CREATE TRIGGER trg_technical_materials_translations_updated
    BEFORE UPDATE ON public.technical_materials_translations
    FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'trg_news_categories_translations_updated') THEN
